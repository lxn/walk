...
type PageOrientation int16

const (
	orientUnspecified PageOrientation = 0
	OrientPortrait    PageOrientation = DMORIENT_PORTRAIT
	OrientLandscape   PageOrientation = DMORIENT_LANDSCAPE
)
...
func (p *PageInfo) Bounds() (val Rectangle, err os.Error) {
	defer func() {
		if x := recover(); x != nil {
			err = toError(x)
		}
...
func (p *PageInfo) Orientation() (val PageOrientation, err os.Error) {
	defer func() {
		if x := recover(); x != nil {
			err = toError(x)
		}
	...
	return PageOrientation(p.printerInfo.valueFromDevMode(dmOrientation, DMORIENT_PORTRAIT)), nil
}

func (p *PageInfo) SetOrientation(value PageOrientation) (err os.Error) {
...
func (p *PageInfo) Margins() PageMargins {
	return p.margins
}

func (p *PageInfo) SetMargins(value PageMargins) {
	p.margins = value
}
...
func (p *PageInfo) boundsFromHDevMode(hDevMode HGLOBAL) Rectangle {
	size := p.paperSizeFallbackHDevMode(hDevMode)

	if p.orientationFallbackHDevMode(hDevMode) == OrientLandscape {
		return Rectangle{Width: size.height, Height: size.width}
	}

	return Rectangle{Width: size.width, Height: size.height}
}

func (p *PageInfo) orientationFallbackHDevMode(hDevMode HGLOBAL) PageOrientation {
	if p.orientation != orientUnspecified {
		return p.orientation
	}

	return PageOrientation(p.printerInfo.valueFromHDevMode(dmOrientation, DMORIENT_PORTRAIT, hDevMode))
}
...
func (p *PageInfo) initFromHDevMode(hDevMode HGLOBAL) (err os.Error) {
	...
	p.orientation = PageOrientation(devMode.DmOrientation)
